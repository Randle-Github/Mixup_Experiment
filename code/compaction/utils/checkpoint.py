...
from compaction.utils.c2_model_loading import get_name_convert_func
...
def save_checkpoint(path_to_job, model, optimizer, epoch, cfg, loss_scaler=None, latest=False):
    """
    Save a checkpoint.
    Args:
    ...
    if isinstance(optimizer, list): # HACK: 如果有多个 optimizer, 只保存第一个的参数
    ...
    if latest:
        path_to_checkpoint = os.path.join(get_checkpoint_dir(path_to_job), 'latest.pyth')
    else:
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        if isinstance(optimizer, list): #HACK: 同样的处理
            optimizer = optimizer[0]
