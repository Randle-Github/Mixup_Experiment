...
class Sthelse(torch.utils.data.Dataset):
    ...
    def load_frame(self, vid_name, frame_idx):
        aug_path = osp.join(self.cfg.DATA.PATH_TO_DATA_DIR, 'frames_aug', vid_name+'_cutmix',
                         '%04d.jpg' % (frame_idx + 1))
        if self.cfg.MIXUP.ENABLE and os.path.exists(aug_path):
            file_path = aug_path
            ...
            for box_anno in frame_anno['labels']:
                ...
                try:
                    ...
                    if box_anno['standard_category'] != 'hand':
                        ...
                        obj_cat = box_anno['category']
                        meta['obj_category'][global_box_id] = self.anno_obj_map[obj_cat]['id']
                except IndexError:
                    pass
