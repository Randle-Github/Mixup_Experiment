...
def load_pkl_from_file(fpath):
    with open(fpath, 'rb') as f:
        obj = pickle.load(f)
    return obj

def ensure_home_path(fpath):
    _, folder = fpath.split('FSL_Datasets/')
    home_path = "/mnt/data0/home/lxs"
    if os.path.exists(home_path):
        return os.path.join(home_path, 'FSL_Datasets', folder)
    home_path = home_path.replace('lxs', 'lixunsong')
    if os.path.exists(home_path):
        return os.path.join(home_path, 'FSL_Datasets', folder)
    home_path = '/opt/data/private/'
    if os.path.exists(home_path):
        return os.path.join(home_path, 'FSL_Datasets', folder)
    home_path = '/data0/lixunsong/'
    return os.path.join(home_path, 'FSL_Datasets', folder)

def convert_bbox(bbox, scale_resize, crop, scale_crop, offset):
    x0, y0, x1, y1 = bbox
    scale_resize_w, scale_resize_h = scale_resize
    scale_crop_w, scale_crop_h = scale_crop
    crop_w, crop_h = crop
