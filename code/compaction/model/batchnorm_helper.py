...
def get_norm(cfg):
    ...
    elif cfg.BN.NORM_TYPE == 'freeze_batchnorm':
        return partial(FrozenBatchNorm3d, track_running_stats=False, affine=False)
    else:
...
class NaiveSyncBatchNorm3d(nn.BatchNorm3d):
    def __init__(self, num_sync_devices, **args):
        ...
        if self.num_sync_devices > 0:
            ...
            self.num_groups = du.get_local_size() // self.num_sync_devices # 4 / 4
        else:
            self.num_sync_devices = du.get_local_size()  # 设置为 0 的话，是不是是一样的？
            self.num_groups = 1
...
class _FrozenBatchNorm(nn.Module):
    def __init__(self, num_features, eps=1e-5, affine=True, track_running_stats=True, momentum=None):
